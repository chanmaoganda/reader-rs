...
* **PR3 — `layout` engine, off-thread.** Minimal CSS subset (block/inline, headings, em/strong, p, br, img, ul/ol/li, blockquote, font-size/weight/style/family, text-align, margin). XHTML → DOM → styled tree → cosmic-text Buffer → paginated glyph runs. Snapshot tests on a fixture chapter. Hits the 200 ms paginate budget in `benches/`.
